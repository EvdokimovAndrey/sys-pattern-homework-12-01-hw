# Домашнее задание к занятию "`Резервное копирование баз данных`" - `Евдокимов Андрей`
## Задание 1. Резервное копирование
### Кейс
Финансовая компания решила увеличить надёжность работы баз данных и их резервного копирования.<br>
Необходимо описать, какие варианты резервного копирования подходят в случаях:<br>
...
1.1. В этом случае подойдет еженедельное полное резервное копирование: создается полная копия базы данных, например, в воскресенье ночью. В сочетании с ежедневным инкрементальным резервным копированием, каждый день создается копия только тех данных, которые изменились с момента последнего полного или инкрементального бэкапа. Это быстрее и меньше нагружает систему, чем ежедневный полный бэкап.<br>
1.2. В данном случае придется спроектировать и обеспечивать систему, поддерживающую возможность содания бэкапо в с повышленной частотой. Полное резервное копирование раз в сутки, а инкрементное - каждый час.<br>
1.3. Да, этого можно достичь за счет репликации.<br>
...
2.1.* Возможно ли автоматизировать этот процесс? Если да, то как?<br>
Приведите ответ в свободной форме.<br>
...
`-F c`: Формат вывода `c` (custom) — сжатый и поддерживает selective restore<br>
`-f /backups/...`: Путь к выходному файлу<br>

2.2. Реализация автоматизации возможна, например, с помощью cron, который будет запускать bash скрипт в заданное время.<br>
...
# Домашнее задание к занятию "`Репликация и масштабирование. Часть 2`" - `Евдокимов Андрей`
## Задание 1
...
### Активный master-сервер и пассивный репликационный slave-сервер
Основным преимуществом этого метода с точки зрения масштабирования будет повышение отказоустойчивости - в случае выхода из строя master-сервера slave-сервер можно перевести в режим мастера (failover). А также преимщуеством станет снятие нагрузки с master-сервера, т.к операции на чтение (или их часть) можно переложить на slave-сервер. Также к преимуществам можно добавить, что на slave-сервере можно проводить процедуру создания резервной копии, не занимая для этого master, и то, что схема с одним master-сервером и с одним slave-сервером относительна проста в реализации.

### Master-сервер и несколько slave-серверов
Данный метод включает в себя почти все преимщуства предыдущего, но еще лучше реализует масштабируемость и отказоустойчивость поскольку:
- Входящий трафик на чтение распределяется между master-сервером и несколькими slave-серверами. Это позволяет обслуживать гораздо больше одновременных пользователей;
- Наличие нескольких slave-серверов делает систему более устойчивой к сбоям;
- slave-серверы можно размещать в разных дата-центрах или регионах. Это позволяет уменьшить сетевую задержку для пользователей из разных частей мира.<br>

Однако второй метод сложнее в реализации и поддержке в сравнении с первым. Требуются дополнительные ресурсы и механизмы для балансировки нагрузки между slave-серверами.
...
### Принципы построения системы и их разграничение
Мы можем разделить три таблицы на отдельные серверы, сгруппировав их по "сущностям" и "транзакциям". Вертикальный шардинг можно использовать для вынесения таблицы книг на группу серверов - для транзакций. Таблицы с пользователями и магазинами содержат относительно статичные данные и их можно вынести в группу серверов - для сущностей.<br>
Горизонтальный шардинг было бы логичным использовать для большой таблицы книг. В качестве ключа шардинга можно использовать ID, например, в случае двух шардов - распределить по четным и нечетным ID.
### Схема
...
|--------------|---------------|
| Master принимает все операции записи (INSERT, UPDATE, DELETE), а slave копирует данные с master и используется только для чтения (SELECT) | Оба сервера равноправны и принимают операции как на чтение, так и на запись, реплицируя данные друг у друга |
| Простая настройка и обслуживание | Сложная настройка, возможны конфликты данных |
| Единая точка отказа (Master) | Высокая доступность, нет единой точки отказа |
