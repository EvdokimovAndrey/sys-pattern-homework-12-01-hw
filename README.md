...
### Ключевые различия:
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
В режиме репликации master-slave master принимает все операции записи (INSERT, UPDATE, DELETE), |а slave копирует данные с master и используется только для чтения (SELECT). |
а slave копирует данные с master и используется только для чтения (SELECT).                     |а slave копирует данные с master и используется только для чтения (SELECT). |
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
